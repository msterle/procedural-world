...
SeperableFilter::SeperableFilter(vector<float> kernel) : interTex(NULL), ownInterTex(false) {
	this->kernel = kernel;
...
BlurFilter::BlurFilter(unsigned int size) : interTex(NULL), ownInterTex(false) {
...
// render quad through filter and capture in frame buffer
void BlurFilter::apply(Texture* inTex, Texture* outTex, Texture* interTex) {
	if(interTex == NULL) {
		ownInterTex = true;
		interTex = new Texture(outTex);
	}
...
void BlurFilter::bind(Texture* inTex, Texture* outTex, Texture* interTex) {
	// delete existing intermediate texture if have ownership
	if(this->interTex != NULL && ownInterTex) {
		delete interTex;
		ownInterTex = false;
